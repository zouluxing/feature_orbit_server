...
// FeatureRepository defines persistence operations for Feature.
...
// NewFeatureRepository constructs a GORM-backed FeatureRepository.
...
func (r *featureRepo) SoftDelete(ctx context.Context, id uint64) error {
	now := time.Now()
	return r.db.WithContext(ctx).Model(&model.Feature{}).
		Where("id = ? AND deleted_at IS NULL", id).
