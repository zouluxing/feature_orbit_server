...
func (p *permRepo) FindByIDs(ctx context.Context, ids []int64) ([]model.Permission, error) {
	var perms []model.Permission
	err := p.db.WithContext(ctx).Where("id IN ?", ids).Find(&perms).Error
	return perms, err
}
...
func (r *oauth2Repo) FindClientsByOwner(ctx context.Context, ownerUserID int64) ([]model.OAuth2Client, error) {
	...
	err := r.db.WithContext(ctx).Where("owner_user_id = ? AND deleted_at IS NULL", ownerUserID).Find(&clients).Error
	return clients, err
}
