...
// MFAEncryptor is the interface for AES-GCM MFA secret encryption (pkg/crypto.AESEncryptor).
...
type authService struct {
	...
	cfg      *config.Config
	enc      MFAEncryptor // nil in dev mode
}
...
func (s *authService) Register(ctx context.Context, req dto.RegisterRequest) (*dto.RegisterResponse, error) {
	if existing, err := s.userRepo.FindByEmail(ctx, req.Email); err != nil { return nil, apperr.ErrInternal } else if existing != nil { return nil, apperr.ErrDuplicateUser }
	if existing, err := s.userRepo.FindByUsername(ctx, req.Username); err != nil { return nil, apperr.ErrInternal } else if existing != nil { return nil, apperr.ErrDuplicateUser }
...
func (s *authService) Login(ctx context.Context, req dto.LoginRequest) (*dto.TokenResponse, error) {
	...
	if err != nil { return nil, apperr.ErrInternal }
	if user == nil { s.cache.Incr(ctx, ipKey, time.Duration(s.cfg.RateLimit.LoginWindowSecs)*time.Second); return nil, apperr.ErrInvalidCredentials }
...
// verifyTOTP decrypts the stored secret (when enc is set) before TOTP validation.
