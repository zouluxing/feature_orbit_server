...
// jwtIssuer is the interface subset of utils.JWTManager used by oauth2Service.
type jwtIssuer interface {
	IssueAccessToken(userUUID, email string, roles, scopes []string) (string, string, error)
...
func NewOAuth2Service(oauthRepo repository.OAuth2Repository, userRepo repository.UserRepository, jwt jwtIssuer, cfg *config.Config) OAuth2Service {
...
func (s *oauth2Service) CreateClient(ctx context.Context, ownerUserID int64, req dto.CreateClientRequest) (*dto.CreateClientResponse, error) {
	...
	if err := s.oauthRepo.CreateClient(ctx, c); err != nil {
...
func (s *oauth2Service) UpdateClient(ctx context.Context, ownerUserID, clientID int64, req dto.UpdateClientRequest) (*dto.OAuth2ClientInfo, error) {
	...
	for i := range clients {
		if clients[i].ID == clientID { target = &clients[i]; break }
	}
	if target == nil { return nil, apperr.ErrClientInvalid }
	if req.Name != nil { target.Name = *req.Name }
	if len(req.RedirectURIs) > 0 { b, _ := json.Marshal(req.RedirectURIs); target.RedirectURIs = string(b) }
	if len(req.Scopes) > 0     { b, _ := json.Marshal(req.Scopes);       target.Scopes = string(b) }
...
func (s *oauth2Service) RotateClientSecret(ctx context.Context, ownerUserID, clientID int64) (string, error) {
	...
	if err := s.oauthRepo.UpdateClient(ctx, &model.OAuth2Client{ID: clientID, ClientSecretHash: string(hash)}); err != nil {
...
func (s *oauth2Service) Token(ctx context.Context, req dto.OAuth2TokenRequest) (*dto.TokenResponse, error) {
	...
	if err := bcrypt.CompareHashAndPassword([]byte(client.ClientSecretHash), []byte(req.ClientSecret)); err != nil {
		return nil, apperr.ErrClientInvalid
	}
	switch req.GrantType {
	case "authorization_code": return s.handleAuthCodeGrant(ctx, req)
	case "refresh_token":      return s.handleRefreshGrant(ctx, req)
	default:                   return nil, apperr.ErrGrantTypeInvalid
	}
}

func (s *oauth2Service) handleAuthCodeGrant(ctx context.Context, req dto.OAuth2TokenRequest) (*dto.TokenResponse, error) {
