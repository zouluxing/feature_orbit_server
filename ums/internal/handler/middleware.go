...
func JWTAuth(jwt *utils.JWTManager, cache service.CacheService) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			response.Fail(c, apperr.ErrTokenExpired)
			c.Abort()
			return
		}
		if cache != nil {
...
func RequirePermission(resource, action string, permSvc service.PermissionService) gin.HandlerFunc {
	return func(c *gin.Context) {
		uuid, _ := c.Get(ContextKeyUserUUID)
		ok, err := permSvc.CheckPermission(c.Request.Context(), uuid.(string), resource, action)
		if err != nil || !ok {
			response.Fail(c, apperr.ErrForbidden)
			c.Abort()
			return
